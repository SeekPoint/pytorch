...
但是 RunResult 注释之中注明：如果结果成功（例如is_failed() = False），则return_values字段包含此代理管理的工作进程的输出（返回值），这些工作进程由其GLOBAL ranks映射。
即，result.return_values[0]是全局 rank 0的返回值。所以，在 _monitor_workers 之中会有一个从 local rank 到 gloabl rank 的映射，我们后续会讲到。
'''
@dataclass
class RunResult:
    """
...
我们接下来就具体看看如何处理"Worker Failure"。
                ...
                if self._remaining_restarts > 0: # 重试
                    ...
                    self._exit_barrier()
                    return run_result
            elif state == WorkerState.HEALTHY:  # 程序正常运行
    ...
    实际上，几乎不可能保证DDP的所有工人都能保证同时结束，所以因此TE提供了一个finalization barrier，这个barrier的作用是对worker finalization 实施等待超时（5分钟）。
